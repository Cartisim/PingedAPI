...
extension User {
    convenience init(from register: RegisterAdminRequest, hash: String) throws {
        self.init(fullName: register.fullName, email: register.email.lowercased(), passwordHash: hash, isAdmin: register.isAdmin, isEmailVerified: register.isEmailVerified)
    }
}
