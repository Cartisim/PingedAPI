...
struct AuthenticationController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        routes.group(Constants.shared.AUTH_ROUTE) { auth in
            auth.post(Constants.shared.REGISTER, use: register)
            auth.post(Constants.shared.LOGIN, use: login)
            ...
            auth.group(Constants.shared.REST_PASSWORD) { resetPasswordRoutes in
                ...
                resetPasswordRoutes.get(Constants.shared.VERIFY, use: verifyResetPasswordToken)
            }
            auth.post(Constants.shared.RECOVER, use: recoverAccount)
            
            auth.post(Constants.shared.ACCESS_TOKEN, use: refreshAccessToken)
            
            // Authentication required
            auth.group(UserAuthenticator()) { authenticated in
                authenticated.get(Constants.shared.CURRENT_USER, use: getCurrentUser)
            }
        }
    }
    ...
    private func getCurrentUser(_ req: Request) throws -> EventLoopFuture<User.Output> {
